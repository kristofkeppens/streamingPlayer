...
{if(this.id=='height')
{if(value&&value!='0')
value=Math.round(CKEDITOR.config.streamingPlayer.width*(value/CKEDITOR.config.streamingPlayer.height));if(!isNaN(value))
dialog.setValueOf('info','width',value);}
else
{if(value&&value!='0')
value=Math.round(CKEDITOR.config.streamingPlayer.height*(value/CKEDITOR.config.streamingPlayer.width));if(!isNaN(value))
...
{var width=dialog.getValueOf('info','width'),height=dialog.getValueOf('info','height'),originalRatio=CKEDITOR.config.streamingPlayer.width*1000/CKEDITOR.config.streamingPlayer.height,thisRatio=width*1000/height;dialog.lockRatio=false;if(!width&&!height)
...
ratioButton.addClass('cke_btn_unlocked');var lang=dialog._.editor.lang.streamingPlayer,label=lang[dialog.lockRatio?'unlockRatio':'lockRatio'];ratioButton.setAttribute('title',label);ratioButton.getFirst().setText(label);return dialog.lockRatio;};var resetSize=function(dialog)
{dialog.setValueOf('info','width',CKEDITOR.config.streamingPlayer.width);dialog.setValueOf('info','height',CKEDITOR.config.streamingPlayer.height);};function loadValue(objectNode,embedNode,paramMap)
...
{var attrDef=attributes[i];switch(attrDef.type)
{case ATTRTYPE_OBJECT:if(!objectNode)
continue;var value=this.getValue();if(isRemove)
objectNode.removeAttribute(attrDef.name);else
if(attrDef.name=='flashvars')
{value=unescape(value);value='config={"clip":{"url":"'+escape(value)+'", "autoPlay":false}'+CKEDITOR.config.streamingPlayer.extraFlashVars+'}';objectNode.setAttribute(attrDef.name,value);}
else
{objectNode.setAttribute(attrDef.name,value);}
break;case ATTRTYPE_PARAM:if(!objectNode)
continue;value=this.getValue();if(isRemove)
...
if(attrDef.name=='flashvars')
{value=unescape(value);value='config={"clip":{"url":"'+escape(value)+'", "autoPlay":false}'+CKEDITOR.config.streamingPlayer.extraFlashVars+'}';embedNode.setAttribute(attrDef.name,value);}
else
{embedNode.setAttribute(attrDef.name,value);}}}}
CKEDITOR.dialog.add('streamingPlayer',function(editor)
{var makeObjectTag=!editor.config.flashEmbedTagOnly,makeEmbedTag=editor.config.flashAddEmbedTag||editor.config.flashEmbedTagOnly;var previewPreloader,previewAreaHtml='<div>'+CKEDITOR.tools.htmlEncode(editor.lang.common.preview)+'<br>'+'<div id="FlashPreviewLoader" style="display:none"><div class="loading"> </div></div>'+'<div id="FlashPreviewBox"></div></div>';return{title:editor.lang.streamingPlayer.title,minWidth:420,minHeight:310,onShow:function()
...
{if(makeObjectTag)
{objectNode=CKEDITOR.dom.element.createFromHtml('<cke:object></cke:object>',editor.document);var attributes={classid:'clsid:d27cdb6e-ae6d-11cf-96b8-444553540000',movie:CKEDITOR.config.streamingPlayer.movie,id:'flowplayer'};objectNode.setAttributes(attributes);}
if(makeEmbedTag)
{embedNode=CKEDITOR.dom.element.createFromHtml('<cke:embed></cke:embed>',editor.document);embedNode.setAttributes({type:'application/x-shockwave-flash',src:CKEDITOR.config.streamingPlayer.movie});if(objectNode)
...
updatePreview(evt.data.value);});this.getInputElement().on('change',function(evt){updatePreview(this.getValue());},this);}},{type:'button',id:'browse',filebrowser:'info:flashvars',hidden:true,style:'display:inline-block;margin-top:10px;',label:editor.lang.common.browseServer}]}]},{type:'hbox',widths:['50%','25%','25%','25%'],children:[{type:'hbox',widths:['70%','30%'],children:[{type:'vbox',padding:1,children:[{type:'text',id:'width',style:'width:80px',default:CKEDITOR.config.streamingPlayer.width,labelLayout:'horizontal',label:editor.lang.flash.width,validate:CKEDITOR.dialog.validate.integer(editor.lang.flash.validateWidth),onKeyUp:onSizeChange,onChange:function(){},setup:function(objectNode,embedNode,paramMap,fakeImage)
...
extraStyles.width=this.getValue()+'px';}},{type:'text',id:'height',style:'width:80px',labelLayout:'horizontal',default:CKEDITOR.config.streamingPlayer.height,label:editor.lang.flash.height,validate:CKEDITOR.dialog.validate.integer(editor.lang.flash.validateHeight),onKeyUp:onSizeChange,onChange:function(){},setup:function(objectNode,embedNode,paramMap,fakeImage)
{loadValue.apply(this,arguments);if(fakeImage)
...
{var height=CKEDITOR.config.streamingPlayer.height/CKEDITOR.config.streamingPlayer.width*width;if(!isNaN(height))
